...
function theTables($connection, $event_id) {
	...
	while ( $row1 = mysqli_fetch_row($result1) ) {
		...
		<td class=\"tableDataCell\"><a href=\"index.php?page=event_vehicle&action=mod&id=" . $row1[0] . "\">
		<button id=\"m_button\"><img id=\"m_image\" src=\"resources/edit.png\" /></button></a></td>
		<td class=\"tableDataCell\"><a href=\"index.php?page=event_vehicle&action=del&id=" . $row1[0] . "&event_id=" . $event_id ."\">
...
if ( $event_id === NULL ) {
		echo "<div id=\"startinfo\"><h1>Aby móc dodać użyte pojazdy
...
if ( count($_POST) > 0 ) {
	...
	if ( isset($_GET['action']) && ( $_GET['action'] === "mod" ) ) {
		...
		$mod_id = $_GET['id'];
		$tName = 'e_vehicle';
		$csh = 'kilometry,praca_postuj,praca_autopompa,paliwo,uwagi,vehicle_id,event_id';
	...
	theTables($connection, $_POST['e_vehicle_event_id']);
	...
	if ( isset($_GET['action']) && ( $_GET['action'] === "del" ) ) {
		...
		include('forms/event_vehicle_mod.php');
		...
		include('forms/event_vehicle.php');
